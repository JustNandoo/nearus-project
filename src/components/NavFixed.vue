<template>
  <header class="header font-montserrat fixed w-full bg-transparent h-20 pt-3 pb-3 items-center z-[1000] shadow-lg"
    ...
    <nav class="flex justify-between items-center w-[92%] mx-auto mr-32">
      <div class="ml-60">
        <img class="w-32" :src="scrolled ? scrolledLogo : logo" alt="logo" :class="{'img-change':scrolled}">
      </div>
      <div class="mr-96">
      ...
      <div class="flex items-center justify-between gap-2 mr-48 relative">
        <div class="rounded-full gap-5 flex items-center justify-center cursor-pointer" @click="$emit('toggle-profile-card')">
          <img :src="profilePicture" alt="Profile Picture" class="object-cover rounded-full h-12 w-12">
          <p class="text-xl font-medium text-balck" :class="{'text-change': scrolled}">Halo, Calvin Aprilio Hariyanto</p>
        </div>
      </div>
    </nav>
  </header>
