<template>
  <div>
    ...
    <div class="mx-4 md:mx-8 lg:mx-36 mt-5 flex flex-col lg:flex-row lg:justify-between">
      <div class="flex flex-col mb-4 lg:mb-0">
        <div class="flex items-center gap-3">
          <font-awesome-icon class="text-light-black w-5 h-5" :icon="faMedal" />
          <p class="font-light text-base md:text-lg lg:text-xl">Rekomendasi</p>
        </div>
        <div class="flex flex-col gap-1">
          <h1 class="uppercase font-bold text-xl md:text-2xl lg:text-3xl">{{ product.productname }}</h1>
          <p class="text-black font-light text-base md:text-lg lg:text-xl">{{ product.location }}</p>
          <div class="flex items-center gap-2">
            <font-awesome-icon class="text-light-black w-5 h-5" :icon="faPerson" />
            <p class="text-black font-light text-sm md:text-base lg:text-lg">Kost {{ product.category }}</p>
          </div>
        </div>
      </div>
    ...
    <hr class="my-10 mx-4 md:mx-8 lg:mx-32 border-t-4 border-neutral-300 mb-10">

    <div class="mt-10 mx-4 md:mx-8 lg:mx-32 mb-20">
      <div>
        <h1 class="font-bold text-xl md:text-2xl lg:text-3xl mb-4">Fasilitas Bersama</h1>
        <div v-if="facilities.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div v-for="(facility, index) in formattedFacilities" :key="index">
            <p class="text-black text-base md:text-lg lg:text-xl font-montserrat">• {{ facility }}</p>
          </div>
        </div>
      ...
      <div>
        <h1 class="font-bold text-xl md:text-2xl lg:text-3xl mb-4">Lokasi dan Jarak</h1>
        <div class="flex flex-col lg:flex-row gap-8 lg:gap-16 justify-between">
          <div v-if="product.lat && product.lng" class="w-full lg:w-1/2 lg:pr-4">
          ...
          <div class="w-full lg:w-1/2 container mx-auto py-8 lg:pl-2">
            <div v-for="place in placesWithIcons" :key="place.name" class="flex items-center justify-between py-5">
              <div class="flex items-center gap-4">
                <div class="flex-1">
                  <h2 class="text-base md:text-lg lg:text-xl text-black">{{ place.name }}</h2>
                </div>
              </div>
              <div class="flex-shrink-0 py-2">
                <p class="text-base md:text-lg lg:text-xl text-black">{{ calculateDistance(product.lat, product.lng, place.lat, place.lng) }} KM </p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          <h1 class="font-bold text-xl md:text-2xl lg:text-3xl mb-10">Kamar</h1>
      ...
      <div class="mx-4 md:mx-8 lg:mx-32 my-10">
